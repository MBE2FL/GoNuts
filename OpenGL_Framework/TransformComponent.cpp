...
void TransformComponent::UIUpdate(float deltaTime, UIAnimator * animator)
{
	AddTRS addTRS = animator->getCurrTRS();
	...
	ry = Quaternion(0.0f, toRadians(_localRotation.y), 0.0f);
	rz = Quaternion(0.0f, 0.0f, toRadians(_localRotation.z));
	// Compute the current local rotation. Also add on any local rotation from any current animation.
	Quaternion localRot = rz * ry * rx;
	localRot = addTRS.rot * localRot;
	...
	// Create translation matrix
	mat4 tran;
	tran.translate(_localPosition + addTRS.pos);


	// Create scale matrix
	mat4 scale;
	scale.scale(_scale + addTRS.scale);
